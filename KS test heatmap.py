# -*- coding: utf-8 -*-
"""
Created on Thu Oct 31 16:06:00 2024
...
# Define the gene sets for each pathway (as in your script)
gene_sets = {
    "ros_genes": [
    ...
    "comp1": ['NDUFS1', 'NDUFS2', 'NDUFS3', 'NDUFS7', 'NDUFS8', 'NDUFV1', 'NDUFV2', 'NDUFA1', 'NDUFA10', 'NDUFA11', 'NDUFA12', 'NDUFA13', 'NDUFA2', 'NDUFA3', 'NDUFA5', 'NDUFA6', 'NDUFA8', 'NDUFA9', 'NDUFAB1', 'NDUFB1', 'NDUFB10', 'NDUFB11', 'NDUFB2', 'NDUFB3', 'NDUFB4', 'NDUFB5', 'NDUFB6', 'NDUFB7', 'NDUFC1', 'NDUFC2', 'NDUFS4', 'NDUFS5', 'NDUFS6', 'SDHA', 'SDHB', 'SDHC', 'UQCRB', 'UQCRQ', 'UQCRC1', 'UQCRC2', 'CYC1', 'UQCRFS1', 'UQCRH', 'UQCR10', 'COX4I1', 'COX5A', 'COX5B', 'COX6A1', 'COX6B1', 'COX6C', 'COX7A2', 'COX7B', 'COX7C', 'COX8A'],
...
# Apply filtering to select samples from PBMC and glioblastoma
...
# Loop over each gene set and perform the KS test
for pathway, genes in gene_sets.items():
    # Check if genes are in the dataset and subset accordingly
    available_genes = [gene for gene in genes if gene in filtered_adata.var_names]
    if not available_genes:
        print(f"No available genes for {pathway} in the dataset.")
        continue

    # Subset the data for the gene set
    adata_pathway = filtered_adata[:, available_genes]

    # Calculate mean expression for each group and flatten to 1D
    mean_expression_gbm = adata_pathway[adata_pathway.obs['source'] == 'glioblastoma'].X.mean(axis=1).A1
    mean_expression_pbmc = adata_pathway[adata_pathway.obs['source'] == 'PBMC'].X.mean(axis=1).A1

    # Perform Kolmogorov-Smirnov (K-S) test
    ks_stat, p_val = ks_2samp(mean_expression_gbm, mean_expression_pbmc)
    ...
    results.append({
        ...
        'p-value': p_val,
        'GBM Mean Expression': mean_expression_gbm.mean(),
        'PBMC Mean Expression': mean_expression_pbmc.mean(),
        'Significance': '*' if p_val < 0.05 else 'non-sig'
    })
...
excel_file_path = r"C:\Users\aroche6\abc\heatmap_KS_Test_All_Pathways.xlsx"
with pd.ExcelWriter(excel_file_path) as writer:
    results_df.to_excel(writer, sheet_name='K-S Test Results for All Pathways')
